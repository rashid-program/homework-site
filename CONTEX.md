...
## Текущий этап: Наполнение базы заданий

### Статус (2026-03-24)
- ✅ Готово: `parse-tasks.mjs` — скрипт парсинга PDF → скриншоты + ответы
- ✅ Обработано: `база заданий/Задание 6.pdf` → `подготовка к базе/Задание 6/`
- 🔜 Следующее: запустить скрипт на остальных PDF (Задание 7, 8 ... 25 — по мере добавления файлов)
- 🔜 После: импорт скриншотов из `подготовка к базе/` в БД через API или seed-скрипт

### Скрипт парсинга: `parse-tasks.mjs`
Запуск: `node parse-tasks.mjs "база заданий/Задание N.pdf"`

**Что делает:**
1. Читает PDF через `pdf-parse`
2. Парсит структуру: `Задача N` (прототип) → `1. 2. 3.` (задания)
3. Конвертирует дроби в LaTeX: `\dfrac{num}{den}`, обрабатывает скобки и операторы
4. Создаёт папки: `подготовка к базе/Задание N/1/1.png`, `1/2.png`, ...
5. Скриншотит каждое задание через Puppeteer (KaTeX server-side, без CDN)
6. Сохраняет ответы: `подготовка к базе/Задание N/answers.json`

**Формат папки результата:**
```
подготовка к базе/
└── Задание 6/
    ├── 1/          ← прототип 1
    ...
    ├── 2/
    │   └── ...
    └── answers.json  ← {1: {1: "11", 2: "9", ...}, 2: {...}, ...}
```

**Особенности PDF (Задание 6):**
- 21 прототип, 230 заданий итого
- Дроби: числитель на отдельной строке, знаменатель на следующей
- Операторы (+, −, ·, :) на отдельных строках
- Иногда оператор+число склеены с точкой: `·6.` → парсер обрабатывает
- Десятичная запятая в LaTeX: `1,2` → `1{,}2`

### Следующий шаг после скриншотов
Написать скрипт `import-tasks.mjs` который:
- Читает `подготовка к базе/Задание N/`
- Для каждого PNG создаёт Task в БД через Prisma
- Копирует PNG в `server/uploads/`
- Проставляет typeId (= N), prototype (= номер папки), answer из answers.json
